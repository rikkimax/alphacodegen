 ...
 */
module alphacodegen.pregen1.pegged_markdown_grammar;

void createDMarkdownGrammar(string outputFile, string moduleName) {
    import pegged.grammar;
    import alphacodegen.common.utils : replace;
...
");
    append(outputFile, grammar(MarkdownGrammar).replace("__ctfe", "false"));
}
...
PipeTable <- PipeTableHeader
             PipeTableSeperator
             PipeTableEntry*

PipeTableHeader <- (:'|' ((:Sp PipeTableHeaderItem :Sp) / :Sp :'|')* / :'|') :endOfLine
PipeTableHeaderItem <~ (!endOfLine !' ' !'|' .)+ (' '+ (!endOfLine !' ' !'|' .)+)*

PipeTableSeperator <: '|' ('-'* '|')+ endOfLine

PipeTableEntry <- (:'|' ((:Sp PipeTableEntryCell :Sp) / :Sp :'|')* / :'|') :endOfLine?
PipeTableEntryCell <~ (!endOfLine !' ' !'|' .)+ (' '+ (!endOfLine !' ' !'|' .)+)*
